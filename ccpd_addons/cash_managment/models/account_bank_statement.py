...
class account_bank_statement(models.Model):
    ...
    def create(self, vals, context=None):
        ...
        if vals.get('name', '/') == '/':
            journal_id = vals.get(
                'journal_id', self._default_journal_id(context=context))
            vals['name'] = self._compute_default_statement_name(
                journal_id, context=dict(
    ...
    def write(self, ids, vals, context=None):
        res = super(account_bank_statement, self).write(
            ids, vals, context=context)
        account_bank_st_line_obj = self.env['account.bank.statement.line']

        for statement in self.browse(ids, context):
            for idx, line in enumerate(statement.line_ids):
                account_bank_st_line_obj.write(
                    [line.id], {'sequence': idx + 1}, context=context)
        return res

    def _default_journal_id(self, context=None):
        if context is None:
            context = {}
        journal_pool = self.env['account.journal']
        journal_type = context.get('journal_type', False)
        company_id = self.env['res.company']._company_default_get(
            'account.bank.statement', context=context)
        if journal_type:
            ids = journal_pool.search([
                ('type', '=', journal_type),
    ...
    @api.multi
    def _end_balance(self, ids, name, attr, context=None):
        res = {}
        for statement in self.browse(ids, context=context):
    ...
    def _compute_default_statement_name(self, journal_id, context=None):
        context = dict(context or {})
        obj_seq = self.env['ir.sequence']
        period = self.env['account.period'].browse(
            context.get('period_id') or self._get_period(
                context=context), context=context)
        context['fiscalyear_id'] = period.fiscalyear_id.id
        journal = self.env['account.journal'].browse(journal_id, None)
        return obj_seq.next_by_id(journal.sequence_id.id, context=context)

    def _currency(self, ids, name, args, context=None):
        res = {}
        res_currency_obj = self.env['res.currency']
        res_users_obj = self.env['res.users']
        default_currency = res_users_obj.browse(
            self._uid, context=context).company_id.currency_id
        for statement in self.browse(ids, context=context):
        ...
        for currency_id, currency_name in res_currency_obj.name_get(
                [x for x in res.values()], context=context):
    ...
    def _get_statement(self, ids, context=None):
        result = {}
        for line in self.env['account.bank.statement.line'].browse(
                ids, context=context):
            result[line.statement_id.id] = True
        return result.keys()

    def _all_lines_reconciled(self, ids, name, args, context=None):
        res = {}
        for statement in self.browse(ids, context=context):
            res[statement.id] = all([
                line.journal_entry_id.id or line.account_id.id for line in statement.line_ids])
        return res
    ...
    name = fields.Char(
        string='Reference', states={'draft': [('readonly', False)]},
        readonly=True,  # readonly for account_cash_statement
        copy=False, default="/",
        help="""if you give the Name other then /, its created Accounting
Entries Move will be with same name as statement name. This allows the
statement entries to have the same references than the statement itself""")
    date = fields.Date(
    ...
    #    lambda self, ids, c={}: ids, ['line_ids','move_line_ids','balance_start'], 10),
    #            'account.bank.statement.line': (_get_statement, ['amount'], 10),
    #        },
    #        string="Computed Balance", help='Balance as calculated based on Opening Balance and transaction lines'),
    company_id = fields.Many2one(
        'res.company', string='Company', related='journal_id.company_id',
        default=lambda self: self.env['res.company']._company_default_get(
            'account.bank.statement'),
        store=True, readonly=True)
    line_ids = fields.One2many(
        'account.bank.statement.line', 'statement_id', 'Statement lines',
        states={'confirm': [('readonly', True)]}, copy=True)
    move_line_ids = fields.One2many(
        'account.move.line', 'statement_id',
...
\'Confirmed\' status."""),
    currency = fields.Many2one(
        'res.currency', compute="_currency", string='Currency')
    account_id = fields.Many2one(
        'account.account', string='Account',
        releated="journal_id.default_debit_account_id", readonly=True,
        help='used in statement reconciliation domain, but shouldn\'t be used elswhere')
    cash_control = fields.Boolean(
        'account.journal', related='journal_id.cash_control',
        string='Cash control'),
    all_lines_reconciled = fields.Boolean(
        string='All lines reconciled', compute="_all_lines_reconciled,"),

    def _check_company_id(self, ids, context=None):
        for statement in self.browse(ids, context=context):
    ...
    # @api.onchange('date')
    # def onchange_date(self, ids, date, company_id, context=None):
    #    """
    ...
    #    pids = period_pool.find(dt=date, context=ctx)
    #    if pids:
    ...
    @api.multi
    def button_dummy(self, ids, context=None):
        return self.write(ids, {}, context=context)

    def _prepare_move(self, st_line, st_line_number, context=None):
    ...
    def _get_counter_part_account(self, st_line, context=None):
    ...
    def _get_counter_part_partner(self, st_line, context=None):
    ...
    def _prepare_bank_move_line(self, st_line, move_id, amount, company_currency_id, context=None):
        ...
        """
        account_id = self._get_counter_part_account(st_line, context=context)
        partner_id = self._get_counter_part_partner(st_line, context=context)
        ...
        return self._prepare_move_line_vals(
            st_line, move_id, debit, credit,
    ...
    def balance_check(self, st_id, journal_type='bank', context=None):
        st = self.browse(st_id, context=context)
    ...
    def statement_close(self, ids, journal_type='bank', context=None):
        return self.write(ids, {'state':'confirm'}, context=context)

    def check_status_condition(self, state, journal_type='bank'):
        return state in ('draft','open')

    def button_confirm_bank(self, ids, context=None):
        if context is None:
            context = {}

        for st in self.browse(ids, context=context):
            j_type = st.journal_id.type
            if not self.check_status_condition(st.state, journal_type=j_type):
                continue

            self.balance_check(st.id, journal_type=j_type, context=context)
            ...
            for st_line in st.line_ids:
                ...
                if st_line.account_id and not st_line.journal_entry_id.id:
                    ...
                    self.env['account.bank.statement.line'].process_reconciliation(
                        st_line.id, [vals], context=context)
                elif not st_line.journal_entry_id.id:
                    raise UserError(_('All the account entries lines must be processed in order to close the statement.'))
                move_ids.append(st_line.journal_entry_id.id)
            if move_ids:
                self.env['account.move'].post(move_ids, context=context)
            self.message_post([st.id], body=_('Statement %s confirmed, journal items were created.') % (st.name,), context=context)
        self.link_bank_to_partner(ids, context=context)
        return self.write(ids,
            {'state': 'confirm', 'closing_date': time.strftime("%Y-%m-%d %H:%M:%S")},
            context=context)
    ...
    def button_cancel(self, ids, context=None):
        ...
        self.env['account.bank.statement.line'].cancel(bnk_st_line_ids, context=context)
        return self.write(ids, {'state': 'draft'}, context=context)

    def _compute_balance_end_real(self, journal_id, context=None):
        res = False
        if journal_id:
            journal = self.env['account.journal'].browse(journal_id, context=context)
    ...
    def onchange_journal_id(self, statement_id, journal_id, context=None):
        if not journal_id:
            return {}
        balance_start = self._compute_balance_end_real(journal_id, context=context)
        journal = self.pool.get('account.journal').browse(journal_id, context=context)
    ...
    def unlink(self, ids, context=None):
        ...
        for item in self.browse(ids, context=context):
            ...
            # been deleted first
            statement_line_obj.unlink([line.id for line in item.line_ids], context=context)
        return super(account_bank_statement, self).unlink(ids, context=context)

    def button_journal_entries(self, ids, context=None):
        ctx = (context or {}).copy()
        ctx['journal_id'] = self.browse(ids[0], context=context).journal_id.id
        return {
        ...
        for statement in self.browse(ids, context=context):
            for st_line in statement.line_ids:
                if st_line.bank_account_id and st_line.partner_id and st_line.bank_account_id.partner_id.id != st_line.partner_id.id:
                    ...
                    bank_vals = bank_obj.onchange_partner_id([st_line.bank_account_id.id], st_line.partner_id.id, context=context)['value']
...
class account_bank_statement_line(models.Model):
    ...
    def create(self, vals, context=None):
        ...
        return super(account_bank_statement_line, self).create(vals, context=context)
    ...
    def unlink(self, ids, context=None):
        ...
        return super(account_bank_statement_line, self).unlink(ids, context=context)
            ...
            if line.journal_entry_id:
                ...
                for aml in line.journal_entry_id.line_id:
                    if aml.reconcile_id:
                        ...
                        self.env['account.move.reconcile'].unlink(
                            [aml.reconcile_id.id], context=context)
                        if len(move_lines) >= 2:
                            self.env['account.move.line'].reconcile_partial(
                                move_lines, 'auto', context=context)
        if move_ids:
            account_move_obj.button_cancel(move_ids, context=context)
            account_move_obj.unlink(move_ids, context)

    @api.multi
    def get_data_for_reconciliations(self, ids, excluded_ids=None, search_reconciliation_proposition=True, context=None):
        """ Returns the data required to display a reconciliation, for each statement line id in ids """
        ret = []
        if excluded_ids is None:
            excluded_ids = []

        for st_line in self.browse(ids, context=context):
            reconciliation_data = {}
            if search_reconciliation_proposition:
                reconciliation_proposition = self.get_reconciliation_proposition(
                    st_line, excluded_ids=excluded_ids, context=context)
            ...
            st_line = self.get_statement_line_for_reconciliation(
    ...
    def get_statement_line_for_reconciliation(self, st_line, context=None):
        """ Returns the data required by the bank statement reconciliation widget to display a statement line """
        if context is None:
            context = {}
        statement_currency = st_line.journal_id.currency or st_line.journal_id.company_id.currency_id
        rml_parser = report_sxw.rml_parse('reconciliation_widget_asl', context=context)
    ...
    def _domain_reconciliation_proposition(self, st_line, excluded_ids=None, context=None):
        if excluded_ids is None:
            excluded_ids = []
    ...
    def get_reconciliation_proposition(self, st_line, excluded_ids=None, context=None):
        """ Returns move lines that constitute the best guess to reconcile a statement line. """
        mv_line_pool = self.env['account.move.line']

        # Look for structured communication
        if st_line.name:
            domain = self._domain_reconciliation_proposition(
                st_line, excluded_ids=excluded_ids, context=context)
            match_id = mv_line_pool.search(
                domain, offset=0, limit=2, context=context)
            if match_id and len(match_id) == 1:
                mv_line_br = mv_line_pool.browse(match_id, context=context)
                target_currency = st_line.currency_id or st_line.journal_id.currency or st_line.journal_id.company_id.currency_id
                mv_line = mv_line_pool.prepare_move_lines_for_reconciliation_widget(
                    mv_line_br, target_currency=target_currency,
                    target_date=st_line.date, context=context)[0]
                ...
                if (mv_line['partner_id']):
                    self.write(
                        st_line.id, {'partner_id': mv_line['partner_id']},
                        context=context)
                    mv_line['has_no_partner'] = False
                return [mv_line]
            elif len(match_id) == 0:
                move = self.pool['account.move'].search(
                    [('name', '=', st_line.ref)], limit=1, context=context)
                if move:
                    domain = [('move_id', '=', move[0])]
                    match_recs = self.get_move_lines_for_reconciliation(
                        st_line, excluded_ids=excluded_ids, limit=2,
        ...
        match_id = self.get_move_lines_for_reconciliation(
            st_line, excluded_ids=excluded_ids, offset=0,
        ...
        if amount_field == 'amount_currency' and amount < 0:
            ...
            domain += [(amount_field, '>', 0), (amount_field, '<=', (sign * amount))]
        mv_lines = self.get_move_lines_for_reconciliation(
            st_line, excluded_ids=excluded_ids, limit=5,
    ...
    def get_move_lines_for_reconciliation_by_statement_line_id(
            self, st_line_id, excluded_ids=None, str=False,
            offset=0, limit=None, count=False, additional_domain=None, context=None):
        ...
        if additional_domain is None:
            additional_domain = []
        st_line = self.browse(st_line_id, context=context)
        return self.get_move_lines_for_reconciliation(
            st_line, excluded_ids, str, offset, limit,
            count, additional_domain, context=context)

    def _domain_move_lines_for_reconciliation(self, st_line, excluded_ids=None, str=False, additional_domain=None, context=None):
        if excluded_ids is None:
            excluded_ids = []
        if additional_domain is None:
            additional_domain = []
    ...
    def get_move_lines_for_reconciliation(self, st_line, excluded_ids=None, str=False, offset=0, limit=None, count=False, additional_domain=None, context=None):
        ...
        while True:
            line_ids = mv_line_pool.search(
                domain, offset=actual_offset, limit=limit,
                order="date_maturity asc, id asc", context=context)
            lines = mv_line_pool.browse(line_ids, context=context)
        ...
        # Or return list of dicts representing the formatted move lines
            ...
            target_currency = st_line.currency_id or st_line.journal_id.currency or st_line.journal_id.company_id.currency_id
            mv_lines = mv_line_pool.prepare_move_lines_for_reconciliation_widget(
                lines, target_currency=target_currency,
    ...
    def get_currency_rate_line(self, st_line, currency_diff, move_id, context=None):
        if currency_diff < 0:
    ...
    def _get_exchange_lines(self, st_line, mv_line, currency_diff, currency_id, move_id, context=None):
        '''
    ...
    def process_reconciliations(self, data, context=None):
        for datum in data:
            self.process_reconciliation(datum[0], datum[1], context=context)

    def process_reconciliation(self, id, mv_line_dicts, context=None):
        ...
        if st_line.journal_entry_id.id:
            raise ValidationError(_('The bank statement line was already reconciled.'))
        for mv_line_dict in mv_line_dicts:
            for field in ['debit', 'credit', 'amount_currency']:
                if field not in mv_line_dict:
                    mv_line_dict[field] = 0.0
            if mv_line_dict.get('counterpart_move_line_id'):
                mv_line = aml_obj.browse(mv_line_dict.get('counterpart_move_line_id'), context=context)
                if mv_line.reconcile_id:
                    raise ValidationError(_('A selected move line was already reconciled.'))

        # Create the move
        move_name = (st_line.statement_id.name or st_line.name) + "/" + str(st_line.sequence)
        move_vals = bs_obj._prepare_move(st_line, move_name, context=context)
        move_id = am_obj.create(move_vals, context=context)
        ...
        if st_line.statement_id.currency.id != company_currency.id:
            if st_line.currency_id == company_currency:
                ...
                amount = currency_obj.compute(
                    ...
                    company_currency.id, st_line.amount, context=ctx)
        else:
            amount = st_line.amount
        bank_st_move_vals = bs_obj._prepare_bank_move_line(
            st_line, move_id, amount, company_currency.id, context=context)
        aml_obj.create(bank_st_move_vals, context=context)
        # Complete the dicts
        ...
        for mv_line_dict in mv_line_dicts:
            ...
            if mv_line_dict.get('counterpart_move_line_id'):
                mv_line = aml_obj.browse(mv_line_dict['counterpart_move_line_id'], context=context)
            ...
            if st_line_currency.id != company_currency.id:
                ...
                if st_line.currency_id and statement_currency.id == company_currency.id and st_line_currency_rate:
                    debit_at_current_rate = self.env['res.currency'].round(
                        company_currency, mv_line_dict['debit'] / st_line_currency_rate)
                    credit_at_current_rate = self.env['res.currency'].round(
                        company_currency, mv_line_dict['credit'] / st_line_currency_rate)
                elif st_line.currency_id and st_line_currency_rate:
                    debit_at_current_rate = currency_obj.compute(
                        statement_currency.id, company_currency.id,
                        mv_line_dict['debit'], st_line_currency_rate, context=ctx)
                    credit_at_current_rate = currency_obj.compute(
                        statement_currency.id, company_currency.id,
                        mv_line_dict['credit'] / st_line_currency_rate, context=ctx)
                else:
                    debit_at_current_rate = currency_obj.compute(
                        st_line_currency.id, company_currency.id,
                ...
                if mv_line_dict.get('counterpart_move_line_id'):
                    ...
                    if debit_at_old_rate - debit_at_current_rate:
                        ...
                        to_create.append(self.get_currency_rate_line(
                            st_line, -currency_diff, move_id, context=context))
                    ...
                    if mv_line.currency_id and mv_line_dict['currency_id'] == mv_line.currency_id.id:
                        ...
                        amount_unreconciled = currency_obj.compute(
                            company_currency.id, mv_line_dict['currency_id'],
                    ...
                    if float_is_zero(mv_line_dict['amount_currency'] + amount_unreconciled, precision_rounding=mv_line.currency_id.rounding):
                        ...
                        if not company_currency.is_zero(currency_rate_difference):
                            exchange_lines = self._get_exchange_lines(
                                st_line, mv_line, currency_rate_difference,
                                mv_line_dict['currency_id'], move_id, context=context)
        ...
        if st_line_currency.id != company_currency.id:
            ...
            if not company_currency.is_zero(diff_amount):
                diff_aml = self.get_currency_rate_line(
                    st_line, diff_amount, move_id, context=context)
        ...
        for mv_line_dict in to_create:
            ...
            new_aml_id = aml_obj.create(mv_line_dict, context=context)
            if counterpart_move_line_id != None:
                move_line_pairs_to_reconcile.append([new_aml_id, counterpart_move_line_id])
        # Reconcile
        for pair in move_line_pairs_to_reconcile:
            # DO NOT FORWARD PORT
            aml_obj.reconcile_partial(pair, context=dict(context, bs_move_id=move_id))
